...
public:
    Job(int id,
        function<void()> task,
        Time when,
        Clock::duration after = Duration::zero()) : id(id), task(task), when(when), after(after), is_recurring(after != Duration::zero()) {}
...
public:
    JobScheduler()
    {
        scheduler_thread = thread(&JobScheduler::run, this);
    }
    ~JobScheduler()
    {
        stop();
...
private:
    void run()
    {
        ...
        while(!is_stopped){
            unique_lock<mutex> lock(mtx);
            if(pq.empty()){
                cv.wait(lock, [this](){ return !pq.empty() || is_stopped; }); // wait until pq has element
            }
            if(is_stopped) break;
            auto next_run_time = pq.top().when;
            cv.wait_until(lock, next_run_time, [this, &next_run_time](){ return is_stopped || pq.top().when < next_run_time; });
...
{
    JobScheduler s;
    s.recurringSchedule([](){cout << "recurr 5 sec\n";}, Clock::now(),chrono::seconds(5));
